...
class Professor {
    ...
    static buscarPgPorMatricula(matricula, callback) {
        const sqlBuscar = `
            SELECT 
                id, 
                matricula_professor, 
    ...
    static cadastrarPagamento(matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario, callback) {
        const sqlInsert = `
            INSERT INTO pagamentos (matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario)
            VALUES (?, ?, ?, ?, ?)
        `;
        conexao.query(
            sqlInsert, 
            [matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario], 
            callback
        );
    }

    static deletarPagamento(id, callback) {
        const sqlDelete = "DELETE FROM pagamentos WHERE id = ?";
        conexao.query(sqlDelete, [id], callback);
    }

    static buscarPagamentoPorId(id, callback) {
        const sqlBuscar = "SELECT id, matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario FROM pagamentos WHERE id = ?";
    ...
    static atualizarPagamento(id, horas_trabalhadas, valor_hora, data_pagamento, salario, callback) {
        const sqlUpdate = `
            UPDATE pagamentos 
            SET horas_trabalhadas = ?, valor_hora = ?, data_pagamento = ?, salario = ? 
            WHERE id = ?
        `;
        conexao.query(
            sqlUpdate, 
            [horas_trabalhadas, valor_hora, data_pagamento, salario, id], 
            callback
        );
    }
