...
app.get("/api/pgprofessores/:matricula", (req, res) => {
    const { matricula } = req.params;

    Professor.buscarPgPorMatricula(matricula, (erro, resultados) => {
        if (erro) {
            console.log("Erro ao buscar pagamentos do professor:", erro);
            return res.status(500).json({ erro: "Erro ao buscar pagamentos do professor." });
        }
        res.json(resultados || []);
    });
});
...
app.post("/cadastrar-pagamento", (req, res) => {
    const { matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario } = req.body;

    Professor.cadastrarPagamento(matricula_professor, horas_trabalhadas, valor_hora, data_pagamento, salario, (erro) => {
        if (erro) {
            console.log("Erro ao lançar pagamento:", erro);
            return res.send("Erro ao lançar pagamento.");
        }
        res.redirect(`/historico-pagamentos.html?matricula=${matricula_professor}`);
    });
});

app.delete("/api/delpagamentos/:id", (req, res) => {
    const { id } = req.params;

    Professor.deletarPagamento(id, (erro) => {
        if (erro) {
...
app.get("/api/pagamentos/:id", (req, res) => {
    ...
    Professor.buscarPagamentoPorId(id, (erro, pagamento) => {
        if (erro) {
            console.log("Erro ao buscar pagamento por ID:", erro);
...
app.post("/atualizar-pagamento", (req, res) => {
    const { id, horas_trabalhadas, valor_hora, data_pagamento, salario } = req.body;

    Professor.atualizarPagamento(id, horas_trabalhadas, valor_hora, data_pagamento, salario, (erro) => {
        if (erro) {
            console.log("Erro ao atualizar pagamento:", erro);
